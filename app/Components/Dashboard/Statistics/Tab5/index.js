...
export default class Tab5 extends Component {
  ...
  countStars() {
    db.transaction(tx => {
      tx.executeSql(
        "SELECT rating FROM table_user ORDER BY date DESC LIMIT 30",
        [],
        (tx, results) => {
  ...
  countLucid() {
    db.transaction(tx => {
      tx.executeSql(
        "SELECT type FROM table_user ORDER BY date DESC LIMIT 30",
        [],
        (tx, results) => {
          var lucid = 0;
          var notLucid = 0;
  ...
  graphData() {
    db.transaction(tx => {
      tx.executeSql(
        "SELECT first_answer,second_answer,fifth_answer,date FROM table_user GROUP BY date ORDER BY date DESC LIMIT 30",
        [],
        (tx, results) => {
          var temp_sleep_data = [];
  ...
  getTags() {
    db.transaction(tx => {
      tx.executeSql("SELECT tags FROM table_user ORDER BY date DESC LIMIT 30", [], (tx, results) => {
        var temp = [];
        for (let i = 0; i < results.rows.length; i++) {
