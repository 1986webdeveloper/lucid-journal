...
export default class Tab1 extends Component {
  ...
  countStars () {
    db.transaction (tx => {
      tx.executeSql(
        "SELECT rating FROM table_user ORDER BY date DESC LIMIT 7",
        [],
        (tx, results) => {
  ...
  countLucid () {
    db.transaction (tx => {
      tx.executeSql(
        "SELECT type FROM table_user ORDER BY date DESC LIMIT 7",
        [],
        (tx, results) => {
          var lucid = 0;
          var notLucid = 0;
          ...
          var entries = { ...this.state.entries };
          if (result_l >= 0)
            entries[0].percentage_lucid = result_l.toFixed(2);
          if (result_nl >= 0)
