...
export default class Question4 extends Component {
  constructor (props) {
    ...
    let audioFileName = 'lucid_audio' + Date.now () + '.aac';
    this.state = {
      audioId: 0,
      counter_state: false,
      isPlay: false,
      ...
      audioPath: General.AUDIO_PATH + audioFileName,
      hasPermission: undefined,
      mediaIcon: Static_Icons.icon_stop,
    ...
    AudioRecorder.requestAuthorization ().then (isAuthorised => {
      ...
      AudioRecorder.onFinished = data => {
        ...
        if (Platform.OS === 'ios') {
          ...
          AsyncStorage.setItem ('voice', this.state.audioFileName);
          AsyncStorage.setItem ('voice_time', JSON.stringify (this.state.record_time));
        }
      };
    });
  }
  ...
  componentWillMount() {
    this.nextPressListener = EventRegister.addEventListener("nextPress", data => {
     this.onStopPress(true);
    }); 
  }

  componentWillUnmount () {
    EventRegister.removeEventListener(this.nextPressListener);
  ...
  async _play(isNextPress) {
    ...
    if(!this.state.isPlay && !this.state.isPause){
      if(!isNextPress){
        var audioSound = new Sound(this.state.audioPath, "", error => {
          if (error) {
            console.log("failed to load the sound", error);
          } else {
            audioSound.play(success => {
              if (success) {
                this.setState({ counter_state: false, audioId: (Number (this.state.audioId) + 1), isPlay: false, isPause: false, mediaIcon: Static_Images.image_play, text_recording: 'Complete' });
              } 
            });
          }
        });
        this.setState({ 
          text_recording: 'Playing',
          sound: audioSound,
          mediaIcon: Static_Icons.icon_stop,
          isPlay: true,
          counter_state: true,
      ...
      if(!isNextPress){
        ...
        this.setState({ counter_state: true, isPause: false, mediaIcon: Static_Icons.icon_stop, text_recording: 'Playing' });
      }
    }
    else{
      this.state.sound.pause();
      this.setState({ counter_state: false, isPause: true, mediaIcon: Static_Images.image_play, text_recording: 'Pause' });
    }
  }
  ...
  _finishRecording (didSucceed, filePath, fileSize) {
    ...
    console.log (
      `Finished recording of duration ${this.state.record_time} seconds at path: ${filePath} and size of ${fileSize || 0} bytes`
    );
  }
  ...
  onStartTimer () {
    let timer = setInterval (() => {
      ...
      this.setState ({
        ...
        record_time: (Number (this.state.record_time) + 1)
      });
    }, 1000);
    this.setState ({timer});
  }

  onStopPress (isNextPress = false) {
    if (this.state.playCount > 0) {
      try {
        if (this.state.play_icon) {
          clearInterval (this.state.timer);
          this.setState ({play_icon: false, mediaIcon: Static_Images.image_play, text_recording: 'Stopped'});
          // this._pause()
          this._stop ();
        }
                    ...
                    <View style={{flex: 1, margin: '5%'}}>
                      ...
                      {renderIf(!this.state.play_icon)(
                        <CountDown
                              ...
                              until={parseInt(this.state.record_time)}
                              componentStyle={{marginTop: -5}}
                              digitTxtStyle={{fontFamily: theme.FONT_MEDIUM, color: 'white'}}
                              timeToShow={['M', 'S']}
                            />
                      )}
                        <Text
