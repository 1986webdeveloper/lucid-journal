...
import { Static_Images, isIphoneX } from "../../Constants";
...
class AppIntroSlider extends React.Component {
  ...
  componentWillMount() {
    ...
    this.done_listener = EventRegister.addEventListener("entryDone", data => {
      setTimeout(() => {
        this._onDonePress();
      }, 500);
    });
  ...
  componentWillUnmount() {
    ...
    EventRegister.removeEventListener(this.done_listener);
  }
      ...
      tx.executeSql(
        ...
        (tx, results) => {
          ...
          if (results.rowsAffected > 0) {
            EventRegister.emit("addEntries", "done");
            AsyncStorage.getItem ('no_of_entries').then (total => {
              //total = 0;
              if (total != null && total > 0) {
                self.props.navigation.pop();
              }
              else{
                self.props.navigation.navigate("SavedFirstEntry");
              }
            });
          } else {
            Alert.alert("Failed");
  ...
  _onNextPress = () => {
    if(this.state.activeIndex == 3){
      EventRegister.emit("nextPress", 1);
    }
